...
export function GlassIconButton({ size = 'md', className, children, ...props }: GlassIconButtonProps) {
  // Igual a SimpleThemeToggle: w-9 h-9 sm:w-10 sm:h-10 md:w-11 md:h-11 y mismo stack visual
  const baseSize = 'w-9 h-9 sm:w-10 sm:h-10 md:w-11 md:h-11';
  const sizeOverride = size === 'sm' ? 'w-9 h-9' : size === 'lg' ? 'w-12 h-12' : baseSize;
  return (
    <button
      {...props}
      className={cn(
        `${sizeOverride} rounded-full flex items-center justify-center relative overflow-hidden`,
